...
const int kChannelMask           = 3;
const int kFirstChannelShift     = 2;
const int kSecondChannelShift    = 4;
const int kThirdChannelShift     = 6;

const int kRedId                 = 0;
const int kGreenId               = 1;
const int kBlueId                = 2;

const int kRed                   = (1 | (kRedId << kFirstChannelShift));
const int kGreen                 = (1 | (kGreenId << kFirstChannelShift));
const int kBlue                  = (1 | (kBlueId << kFirstChannelShift));
const int kBlack                 = 0;

const int kRGBX                  = ((kRed  << 0) | (kGreen << 4) | (kBlue  << 8) | (kBlack << 12));
const int kRBGX                  = ((kRed  << 0) | (kBlue  << 4) | (kGreen << 8) | (kBlack << 12));
const int kBGRX                  = ((kBlue << 0) | (kGreen << 4) | (kRed   << 8) | (kBlack << 12));

const int k_rgba_axis            = 3;

const int k_rgb_mask[3] = int[]( kRGBX, kRBGX, kBGRX );
...
const float k_crt_bloom_strength   = 0.0;
...
vec3 ApplyHDR10(const vec3 hdr_linear)
{
   ...
   vec3 hdr10 = LinearToST2084(max(pq_input, vec3(0.0)));
                  ...
                  const float vertical_bias)
{
   ...
   for(int ch = 0; ch < 3; ch++)
   {
      ...
      float channel_scanline_distance  = clamp(effective_distance / beam_width, 0.0, 1.0);

      vec4 channel_control_points      = vec4(1.0, 1.0, signal_strength * scanline_attack, 0.0);
...
vec3 Scanlines(vec2 texcoord)
{
   ...
   int mask_idx = int(floor(mod(current_position.x, 4.0)));
   int colour_mask = (k_rgb_mask[SubpixelLayout] >> (mask_idx * 4)) & 0xF;
   ...
   vec3 linear_709 = max(scanline_colour, vec3(0.0));

   /* Build mask from subpixel layout bitfield */
   int channel_count = colour_mask & 3;
   vec3 mask = vec3(0.0);
   if(channel_count > 0)
   {
      int ch0 = (colour_mask >> kFirstChannelShift) & 3;
